...
def test_complex_update(git_fleximod, complex_update, logger):
    ...
    assert("ToplevelOptional not checked out, aligned at tag v5.3.2" in normalize_whitespace(status.stdout))
    assert("ToplevelRequired not checked out, aligned at tag MPIserial_2.5.0" in normalize_whitespace(status.stdout))
    assert("AlwaysRequired not checked out, aligned at tag MPIserial_2.4.0" in normalize_whitespace(status.stdout))
    assert("Complex not checked out, out of sync at tag testtag02, expected tag is testtag3" in normalize_whitespace(status.stdout))
    assert("AlwaysOptional not checked out, out of sync at tag None, expected tag is MPIserial_2.3.0" in normalize_whitespace(status.stdout))
    ...
    status = git_fleximod(complex_update, "status")
    assert("ToplevelOptional not checked out, aligned at tag v5.3.2" in normalize_whitespace(status.stdout))
    assert("ToplevelRequired at tag MPIserial_2.5.0" in normalize_whitespace(status.stdout))
    assert("AlwaysRequired at tag MPIserial_2.4.0" in normalize_whitespace(status.stdout))
    assert("Complex at tag testtag3" in normalize_whitespace(status.stdout))
    ...
    assert("AlwaysRequired at tag MPIserial_2.4.0" in normalize_whitespace(status.stdout))
    assert("Complex at tag testtag3" in normalize_whitespace(status.stdout))
    assert("AlwaysOptional at tag MPIserial_2.3.0" in normalize_whitespace(status.stdout))
